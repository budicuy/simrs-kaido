...
<head>
    ...
    <title>Daftar Perawat - Rumah Sakit Islam Banjarmasin</title>
...
<body>
    ...
    <main>
        ...
        <div class="card">
            <div class="content-header">
                <h3>
                    <a href="{{ route('perawat') }}" class="sub-link" style="color: #000; text-decoration: none;">Data Perawat</a>
                </h3>
            </div>
            <div id="loadingMessage" class="loading-message">Memuat data perawat...</div>
            ...
            <div class="top-bar">
                <div class="left-group">
                    <div class="search-group">
                        <input type="text" placeholder="Cari berdasarkan Nama Perawat" id="searchInput" />
            ...
            <div id="noDataMessage" class="info-message" style="display: none;">Tidak ada data perawat.</div>
    ...
    <script>
        const BASE_API_URL = 'https://nazarfadil.me/api'; // Pastikan sesuai URL Laravel Anda
        ...
        // Tampilkan nama dan role user jika ada di localStorage
        ...
        // --- Fungsi untuk Memuat Data Perawat ---
        async function loadPerawats(searchQuery = '') {
            loadingMessage.style.display = 'block';
            errorMessage.style.display = 'none';
            noDataMessage.style.display = 'none';
            perawatTableBody.innerHTML = '';

            // Jika ada query pencarian, API akan memfilternya di backend.
            // Jika tidak, API akan mengembalikan semua data.
            let url = new URL(`${BASE_API_URL}/perawats`);
            if (searchQuery) {
            ...
            try {
                ...
                if (!response.ok) {
                    // Jika token tidak valid (401 Unauthorized), logout paksa
                ...
                // Akses array perawat dari dalam kunci "data"
                const perawats = Array.isArray(result.data) ? result.data : [];

                loadingMessage.style.display = 'none';

                if (perawats.length === 0) {
                    noDataMessage.style.display = 'block';
                    if (searchQuery) {
                        noDataMessage.textContent = `Data Perawat dengan nama "${searchQuery}" tidak ditemukan.`;
                    } else {
                        noDataMessage.textContent = 'Tidak ada data perawat.';
                    }
                    return;
                }

                perawats.forEach((perawat, index) => {
                    const row = document.createElement('tr');
                    // **PERBAIKAN KUNCI UTAMA DI SINI**
                    // Menggunakan `perawat.id` sesuai dengan respons JSON API
                    row.innerHTML = `
                        <td>${index + 1}</td>
                        <td>${perawat.id}</td>
                        <td>${perawat.nama_perawat}</td>
                        <td>${perawat.no_hp_perawat || '-'}</td>
                    `;
                    perawatTableBody.appendChild(row);
                });

            } catch (error) {
                loadingMessage.style.display = 'none';

                // Tangani error otentikasi secara khusus
                if (error.message.includes('Unauthenticated')) {
                    ...
                    errorMessage.textContent = 'Gagal memuat data perawat: ' + error.message;
                    console.error('Error fetching perawats data:', error);
                }
            }
        }
        ...
        searchButton.addEventListener('click', () => {
            ...
            loadPerawats(query); // Panggil fungsi utama dengan query pencarian
        });
